	...
	for(int x =0; x<weeks;x++){
		...
		for(int y =0; y<game_week; y++){
			...
			while(true){
				...
				//getchar();
				//std::cout << tmp[pick][0] << "," << tmp[pick][1] << std::endl;
				//std::cout << !(contains_reverse(pairings,tmp[pick],x,num_teams)) << (!isIn(ls,tmp[pick][0])) << (!isIn(ls,tmp[pick][1])) <<(not isIn(used,pick))<< std::endl;
...
int main(){
	std::vector<std::vector<int> > p= gen_fixtures(20);
	print(p);
